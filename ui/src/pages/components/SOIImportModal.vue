...
const fileType = computed<"table" | "fasta">(() => {
  if (
    fileName.value.endsWith(".tsv") ||
    fileName.value.endsWith(".csv") ||
    fileName.value.endsWith(".xlsx") ||
    fileName.value.endsWith(".xls") ||
...
const sequencesToImport = computed(() => {
  if (fileType.value === "table") {
    ...
    if (fastaData.value === undefined) return undefined;
...
const importData = computed<TextAndRecords>(() => {
  ...
  if (soi === undefined) return { text: `Please select sequences` };
  ...
  if (da.type === "amino-acid" && props.alphabet === "nucleotide")
    return {
      text: `Can't convert amino acid sequence to nucleotides, please create nucleotide sequence list.`,
    };

  let records = recordsToImport.value;
  if (records === undefined) return { text: `Please select sequence name column.` };
