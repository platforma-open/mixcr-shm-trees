...
watch(
  ...
  (e) => {
    data.newBasketName = inferNewName(
      e,
      (i) => `Basket #${i}`,
      data.newBasketName === "" ? undefined : data.newBasketName,
    );
  },
  { immediate: true },
);
...
<template>
  ...
  >
    ...
    <PlTextField
      v-if="data.targetBasket === ''"
      v-model="data.newBasketName"
      label="New Basket Name"
    />
    ...
    <template #actions>
      <PlBtnPrimary @click="runAdd" :disabled="!canAdd"> Add </PlBtnPrimary>
