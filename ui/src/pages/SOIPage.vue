...
watch(
  ...
  (ids) => {
    if (data.currentListId === undefined && ids.length > 0) data.currentListId = ids[0];
      ...
      if (ids.length === 0) data.currentListId = undefined;
      else data.currentListId = ids[0];
    }
  },
  { immediate: true },
);
...
function addNewList() {
  ...
  sois.push({
    ...
    parameters: {
      id,
      name,
      type: data.newList.type,
      targetFeature: data.newList.targetFeature,
      searchParameters: {
...
async function importFile() {
  const file = await getRawPlatformaInstance().lsDriver.showOpenSingleFileDialog({
    ...
    buttonLabel: "Import",
    filters: [{ name: "Fasta Or Table", extensions: ["xlsx", "tsv", "csv", "fa", "fasta", "txt"] }],
  });
  if (!file.file) return;
...
const currentListIdForList = computed<string>({
  ...
  set: (newValue) => {
    if (newValue === "") startAddingNewList();
    else data.currentListId = newValue as PlId;
  },
});
...
<template>
  <PlBlockPage>
    <template #title>
      ...
      <PlBtnGhost v-else @click.stop="() => startAddingNewList()" icon="add"
        >Create new list</PlBtnGhost
      >
    </template>
    <template #append>
      <template v-if="data.currentListId">
        <PlBtnGhost @click.stop="() => (data.listToDelete = data.currentListId)" icon="delete-bin"
          >Delete current list
        </PlBtnGhost>
        <PlBtnGhost @click.stop="importFile" icon="dna-import">Import Sequences</PlBtnGhost>
        <PlBtnGhost @click.stop="() => (data.settingsOpen = true)" icon="settings"
          >Settings</PlBtnGhost
        >
      </template>
    </template>
    ...
    <SOIImportModal
      v-if="data.importFile"
      :file="data.importFile"
      @on-close="data.importFile = undefined"
      @on-import="onImport"
    ...
    <PlDialogModal
      v-if="data.newList !== undefined"
      :model-value="true"
      @update:model-value="
        (v) => {
          if (!v) data.newList = undefined;
        }
      "
    >
      ...
      <PlDropdown
        ...
        v-model="data.newList.targetFeature"
        label="Target Feature"
      />
      <p>
        <b>Note:</b> Nucleotide sequences will be automatically translated if added to the amino
        acid sequence list.
      </p>
      <template #actions>
    ...
    <PlDialogModal
      v-if="data.listToDelete !== undefined"
      :model-value="true"
      @update:model-value="
        (v) => {
          if (!v) data.listToDelete = undefined;
        }
      "
    >
